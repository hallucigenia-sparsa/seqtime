...
compareTS<-function(input.folder="",expIds=c(), sliced.folder="", epsilon=0.2, hurstBins=c(0.5,0.7,0.9), maxautocorBins=c(0.3,0.5,0.8), timeDecaySliceDef=c(1,50), returnDistribs=FALSE, returnTS=FALSE){
  ...
  for(expId in expIds){
    ...
    if(returnDistribs == FALSE && returnTS==FALSE){
      ...
      # auto-correlation slope
      qualOut=limitsQuality(ts,autocorOnly = TRUE)
