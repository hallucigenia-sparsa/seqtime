...
# maxautocorBins: the bins used for the maximal autocorrelation, needed to set labels correctly
# hurstBins: the bins used for the Hurst exponent, needed to set labels correctly
#
# Examples:
# plotTSComparison(table,property="black")

plotTSComparison<-function(data, distribs, colorBy="interval", type="boxplot", property="pink", summary.type="noise", jitter.width=0.1, dot.size=3, pcs=c(1,2), useGgplot=FALSE, skipIntervals=FALSE, skipHighDeathrate=FALSE, addInitAbund=TRUE, scale=FALSE, logscale=FALSE, generator="all", pch="", maxautocorBins=c(0.3,0.5,0.8), hurstBins=c(0.5,0.7,0.9)){
  ...
  if(property=="pink"){
    ...
    descript="R2 of log(dissimilarity) and log(interval)"
  }else if(property=="timedecayslope"){
    descript="slope of log(dissimilarity) and log(interval)"
  }
  ...
  if(type=="boxplot"){
    ...
    if(skipIntervals==TRUE || skipHighDeathrate==TRUE){
      ...
      if(summary.type=="noise"){
        ...
        filteredData=list(data$deaths[keepIndices],data$interval[keepIndices],data$algorithm[keepIndices], data$maxautocorbin1[keepIndices], data$maxautocorbin2[keepIndices],data$maxautocorbin3[keepIndices],data$maxautocorbin4[keepIndices])
        names(filteredData)=c("deaths","interval","algorithm","maxautocorbin1","maxautocorbin2","maxautocorbin3","maxautocorbin4")
      }else if(summary.type=="hurst"){
        filteredData=list(data$deaths[keepIndices],data$interval[keepIndices],data$algorithm[keepIndices], data$lowhurst[keepIndices], data$middlehurst[keepIndices],data$highhurst[keepIndices],data$veryhighhurst[keepIndices])
        names(filteredData)=c("deaths","interval","algorithm","lowhurst","middlehurst","highhurst","veryhighhurst")
      }
    ...
    names=c(rep("ricker",length(ricker)), rep("glv",length(glv)), rep("hubbell",length(hubbell)), rep("dm",length(dm)), rep("davida",length(davida)), rep("davidb",length(davidb)), rep("soi",length(soi)))
    ...
    # with our without slope does not make much difference
